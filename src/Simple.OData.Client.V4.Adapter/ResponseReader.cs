    ...
    public class ResponseReader : ResponseReaderBase
    {
        ...
        protected override void ConvertEntry(ResponseNode entryNode, object entry)
        {
            if (entry != null)
            {
                ...
                foreach (var property in odataEntry.Properties.OfType<ODataProperty>())
                {
        ...
        private object GetPropertyValue(object value)
        {
            ...
            {
                return resource.Properties.OfType<ODataProperty>().ToDictionary(x => x.Name, x => GetPropertyValue(x.Value));
            }
